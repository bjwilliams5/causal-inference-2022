...
read_csv(here("data/NLSY97_clean.csv")) %>%
  ...
  pivot_wider(names_from = race, values_from = incarcerated_pct) %>%
  ...
  kbl(
    caption = "Percent Incarcerated in 2002 by Race and Gender",
    booktabs = TRUE,
    format = "latex",
  ...
  write_lines(here("tables/incarcerationpct_by_racegender.tex"))
